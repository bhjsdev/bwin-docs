...
Events are per-instance: `on` and `off` register and remove listeners on a
single window, and the payload each listener receives depends on the event.
...
affected `<bw-glass>` element (the detached glass element for `detach`).
