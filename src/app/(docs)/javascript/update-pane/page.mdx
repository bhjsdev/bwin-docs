...
Click the button to update the right pane's title and content. See <a href={getPath('/apis/updatePane')}>updatePane</a> API for more details. The example below is hosted within an `iframe`. You can also view it <a href={getPath('/update-pane.html')} target="_blank" rel="noopener noreferrer">here</a>.
